...
install.packages(c("readr","ggplot2", "dplyr", "gridExtra", "stringr", "RColorBrewer","ggpubr","scales","readxl", "vctrs", "rlang"))
...
data <- read_excel("D:/Mestrado/Phylogenetic_analysis/ELEMENT - size and density chart.xlsx")

# stringsAsFactors = FALSE evita que as strings sejam convertidas em fatores

colnames(data) # Verificar os nomes das colunas

# Converter vírgulas em pontos para números decimais
## Primeiro, identifique quais colunas devem ser numéricas
numeric_cols <- c("Genome Size (mb)", "% Quantity of Elements")  # Substitua pelos nomes reais
## Converter as colunas para numéricas após substituir vírgulas por pontos
data <- data %>%
...
p1 <- ggplot(data, aes(x = reorder(Strain, order_index), y = `Genome Size (mb)`, fill = Host)) +
  geom_segment(aes(x = reorder(Strain, order_index), 
                   xend = reorder(Strain, order_index), 
                   y = -2,  # Começa abaixo do eixo X (onde estão os nomes)
                   yend = `Genome Size (mb)`),
               linetype = "dotted", 
               color = "gray50", 
               size = 0.5) +  # Linhas pontilhadas
  ...
  theme(
    ...
    panel.grid.major.y = element_line(linetype = "dotted", color = "gray50", linewidth = 1),  # Linhas pontilhadas horizontais
    panel.grid.minor.y = element_blank(),  # Remove linhas menores
  ...
  coord_cartesian(ylim = c(0, max(data$`Genome Size (mb)`) * 1.3),
                  clip = "off") # Permite desenhar fora da área do plot

print(p1)

# Criar o segundo histograma (% de TEs) ########################################

p2 <- ggplot(data, aes(x = reorder(Strain, order_index), y = `% Quantity of Elements`, fill = Host)) +
  geom_segment(aes(x = reorder(Strain, order_index), 
                   xend = reorder(Strain, order_index), 
                   y = -2,  # Começa abaixo do eixo X
                   yend = `% Quantity of Elements`),
               linetype = "dotted", 
               color = "gray50", 
               linewidth = 0.5) +  # Linhas pontilhadas
  geom_bar(stat = "identity", color = "black", linewidth = 0.3, width = 1) +
  geom_text(aes(y = 0, label = Elements),  # y = 0 para começar na base!!!
            angle = 90, hjust = 0, vjust = 0.5, size = 2, color = "black") +
  scale_fill_manual(values = host_colors) +
  scale_x_discrete(expand = c(0, 0)) +
  theme_minimal() +
  theme(
    ...
    panel.grid.major.x = element_blank(),
    panel.grid.minor.x = element_blank(),
    panel.grid.major.y = element_line(linetype = "dotted", color = "gray50", linewidth = 0.5),  # Linhas pontilhadas horizontais
    panel.grid.minor.y = element_blank(),  # Remove linhas menores
    plot.title = element_text(size = 14, face = "bold", hjust = 0.5),
  ...
  labs(
    ...
    y = "% Quantity of Elements",
    title = "Transposable Elements Distribution",
    fill = "Host"
  ) +
  coord_cartesian(ylim = c(0, max(data$`% Quantity of Elements`) * 1.3),
                  clip = "off")
...
png("D:/histogramas.png", width = 16, height = 20, units = "in", res = 600)
...
ggsave("E:/combined_plot.png", combined_plot, width = 16, height = 20, dpi = 600)

################################################################################

# Ler os dados do Excel
csv <- read_excel("D:/Mestrado/Phylogenetic_analysis/ELEMENT - size and density chart.xlsx")
...
dados_clean <- csv %>%
  ...
  rename(
    ...
    percent_is = `% Quantity of Elements`
  ) %>%
  # Converter vírgulas para pontos E forçar conversão para numérico
  mutate(
    genome_size_bp = as.numeric(gsub(",", ".", as.character(genome_size_bp))),
    percent_is = as.numeric(gsub(",", ".", as.character(percent_is)))
  ) %>%
...
criar_grafico_hospedeiro <- function(host_name, data) {
  ...
  p <- ggplot(dados_host, aes(x = log10_genome_size, y = log10_percent_is)) +
    ...
    labs(
      ...
      y = expression(log[10] ~ "TE density (%)")
    ) +
    theme_classic() +
    theme(
...
if(length(graficos_lista) >= 3) {
  ...
  grafico_combinado <- grid.arrange(grafico_A, grafico_B, grafico_C, 
                                    ncol = 1, 
                                    top = "Genome Size vs TE Density by Host")
  
  # Salvar o gráfico combinado
  ggsave("D:/line_graphs_by_host.png", grafico_combinado, 
         width = 10, height = 20, dpi = 600)
